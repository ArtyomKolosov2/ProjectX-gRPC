...
namespace ProjectX.ConsoleClient
{
    class Program
    {
        static async Task Main(string[] args)
        {
            ...
            var headers = new Metadata { { "Authorization", $"Bearer {reply.Token}" } };

            var greeterClient = new FileUploader.FileUploaderClient(channel);
            using var call = greeterClient.Upload(headers);
            await using var fileStream = new FileStream("C:\\Users\\User\\Desktop\\reshenie.txt", FileMode.OpenOrCreate);
